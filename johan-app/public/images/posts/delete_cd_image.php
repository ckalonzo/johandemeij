...
$query = "SELECT front_cover FROM ck_cds WHERE id = $postID";
$result = mysql_query($query) or die(mysql_error());

while (list($front_cover) = mysql_fetch_row($result)) { 
$myFile = $front_cover;
unlink($myFile);
}

$query1 = "UPDATE ck_cds SET `front_cover` = '' WHERE `ck_cds`.`id` =$postID";
